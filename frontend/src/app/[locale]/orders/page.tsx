...
import { PagedResult } from '@/types/product';
...
export default function OrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold tracking-tight text-ink">Order history</h1>
      <CurrencyDisclaimer variant="history" />
                            ...
                            <div key={item.id} className="p-4 flex items-center justify-between gap-4 text-sm">
                              <div>
                                ...
                                <span className="text-xs text-ink-muted block mt-1">
                                  Qty: {item.quantity} @ {formatAud(item.price)} each
                                </span>
                              </div>
                      ...
                      <div className="flex justify-end pt-2">
                        <div className="w-full sm:w-64 space-y-1.5 text-xs text-ink-muted">
                          <div className="flex justify-between">
                            <span>Subtotal:</span>
                            <span className="tnum font-medium text-ink">{formatAud(details.subtotal)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Shipping & Handling:</span>
                            <span className="tnum font-medium text-ink">{formatAud(details.shippingCost)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>GST (10%):</span>
                            <span className="tnum font-medium text-ink">{formatAud(details.tax)}</span>
                          </div>
                          <div className="border-t border-border pt-1.5 flex justify-between text-sm font-bold text-ink">
                            <span>Grand Total:</span>
                            <span className="tnum text-accent">{formatAud(details.total)}</span>
                          </div>
                        </div>
                      </div>
                    </>
