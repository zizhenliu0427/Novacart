...
public interface IOrderFactory
{
    ...
    Order CreateFromCart(
        ...
        User user,
        UserAddress address,
        IReadOnlyCollection<PriceRule> activeRules,
...
public class OrderFactory : IOrderFactory
{
    ...
    public Order CreateFromCart(
        ...
        IReadOnlyDictionary<Guid, Product> productsById)
    {
        var subtotal = cart.Items.Sum(ci =>
            _pricing.ResolveEffectivePrice(productsById[ci.ProductId], activeRules) * ci.Quantity);
        ...
        {
            var product = productsById[ci.ProductId];
            return new OrderItem
            {
                ProductId = ci.ProductId,
                ProductNameSnapshot = product.Name,
                PriceAtPurchase = _pricing.ResolveEffectivePrice(product, activeRules),
