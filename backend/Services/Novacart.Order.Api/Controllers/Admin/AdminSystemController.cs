...
public class AdminSystemController : ControllerBase
{
    ...
    /// <summary>
    /// Returns RabbitMQ queue depths, highlighting MassTransit <c>_error</c> (DLQ) queues with pending messages.
    /// </summary>
    [HttpGet("messaging")]
    public async Task<IActionResult> GetMessaging(CancellationToken cancellationToken)
    {
        var queues = await _messaging.GetQueuesAsync(cancellationToken);
        var errorQueues = queues.Where(q => q.IsErrorQueue).ToList();
        var poisonCount = errorQueues.Sum(q => q.Messages);
