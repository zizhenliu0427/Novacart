...
| 认证 | JWT(HS256)存放于 HttpOnly cookie + bcrypt 密码哈希 |
| 邮件 | MailKit(SMTP) |
...
| Service | 职责 |
|---|---|
| `AuthService` | 注册 / 登录;bcrypt 哈希;签发 JWT |
| `JwtTokenService` | HS256 token 签名(Singleton,无状态) |
...
| `AdminOrderService` | 管理员订单视图 + 6 态状态机 + 审计历史 + 邮件 |
...
- **Singleton(单例)**(无状态 / 线程安全):`AppDbContext`、`IConnectionMultiplexer`(Redis)、`IRedisCacheService`、`IJwtTokenService`。
- **Scoped(每请求)**:全部业务 service、工厂、策略。
- **Hosted services(托管服务)**:暂无(邮件目前为同步发送 —— 异步队列属于计划中的增强项)。
...
- **JWT 存于 HttpOnly cookie**(`novacart_jwt`、`Secure`、`SameSite=Strict`、`Path=/api`):前端永远接触不到原始 token —— 可防范 XSS。bearer 中间件通过 `OnMessageReceived` 从 cookie 读取,并为 Swagger 保留 Bearer 头回退。
...
- **API 层**:`apiCall` 封装(`lib/api.ts`)注入 `credentials: 'include'`,解析 `ProblemDetails` / 校验错误,并区分 401(过期 → 重定向到登录)与 403(权限错误,停留原处)。
